...
def create_nodes_batch(tx, batch):
    ...
    SET h.name = horse.name,
        h.gender = horse.gender,
        h.birth_year = horse.birth_year,
        h.is_champion = horse.is_champion,
        h.inbreeding_score = horse.inbreeding_score,
        h.defect_risk_score = horse.defect_risk_score
...
def main():
    ...
    horses = silver_df.select("id", "name", "gender", "birth_year", "sire_name", "dam_name", "is_champion", "inbreeding_score", "defect_risk_score") \
        .fillna({"sire_name": "", "dam_name": "", "inbreeding_score": 0.0, "defect_risk_score": 0.0}) \
        .collect()
    ...
    for row in horses:
        horse_list.append({
            ...
            "inbreeding_score": float(row["inbreeding_score"]),
            "defect_risk_score": float(row["defect_risk_score"])
        })
